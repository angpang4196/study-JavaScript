...
<body>
	<h1>자바스크립트의 배열</h1>
	<p>자바스크립트에서는 함수도 변수에 담겨서 사용되다보니 함수 복제도 가능하고, 함수 전달도 가능하다.</p>
	<p>자바스크립트 배열이 가지고 있는 함수중에는 매개변수로 함수를 전달할 수 있는 함수들이 있다. 이걸 고차함수라고
		부른다 / (forEach))</p>
	<%
	if (Math.random() > 1) {
	%>
	<script>
		...
		// filter 전달한 함수에서 true 가 반환되는 애들만 모아서 새로운 배열을 만들어줌.
